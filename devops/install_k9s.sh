...
install_k9s() {
    if [ "$(uname)" = Darwin ]; then
        echo_danger 'error: sorry, MacOS not supported\n';
        return 1;
    fi
    ...
    if [ ! -x "$(command -v gcloud)" ]; then
        echo_danger "error: \"$(basename "${0}")\" requires gcloud, try: 'sudo apt-get install -y google-cloud-sdk'\n"
        return 1
    fi
    ...
    if [ ! -x "$(command -v k9s)" ]; then
        ...
        echo_info 'curl -sS https://webinstall.dev/k9s | bash\n'
    ...
    echo_info 'sudo apt-get install google-cloud-sdk-gke-gcloud-auth-plugin\n'
    sudo apt-get install google-cloud-sdk-gke-gcloud-auth-plugin

    echo_info 'gcloud container clusters get-credentials staging-lemonde --region europe-west1\n'
