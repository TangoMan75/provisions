...
while getopts :v:h OPTION; do
    case "${OPTION}" in
        v) VERSION="${OPTARG}";;
        h) echo_success 'description:' 2 14; echo_primary 'Install vagrant\n'
...
if [ "${INSTALL}" != true ]; then
    echo_danger "error: Cannot install \"vagrant${VERSION}\", invalid version\n" 2
    exit 1
fi
...
if [ ! -x "$(command -v vagrant)" ]; then
    ...
    echo_info "sudo dpkg --install \"vagrant_${VERSION}_x86_64.deb\"\n"
