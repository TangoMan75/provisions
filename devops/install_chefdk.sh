...
if [ -z "${UBUNTU}" ]; then
    if sw_vers -productVersion 2>/dev/null | grep -q '10.12'; then
        ...
        echo_danger 'error: unsupported system, try installing manually: https://downloads.chef.io/chefdk'
        exit 1
    fi
fi
...
if [ -n "${UBUNTU}" ]; then
    ...
    echo_info "sudo dpkg --install \"chefdk_${VERSION}-1_amd64.deb\"\n"
...
if [ -n "${DARWIN}" ]; then
    ...
    echo_info "sudo dpkg --install \"chefdk_${VERSION}-1.dmg\"\n"
