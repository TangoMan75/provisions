...
if [ ! -x "$(command -v gpg)" ]; then
    echo_error "\"$(basename "${0}")\" requires gnupg, try: 'sudo apt-get install -y gnupg'"
    exit 1
fi

if [ ! -x "$(command -v lsb_release)" ]; then
    echo_error "\"$(basename "${0}")\" requires lsb-release, try: 'sudo apt-get install -y lsb-release'"
    exit 1
fi
