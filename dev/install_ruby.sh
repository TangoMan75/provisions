...
while getopts :v:h OPTION; do
    case "${OPTION}" in
        v) VERSION="${OPTARG}";;
        h) echo_success 'description:' 2 14; echo_primary 'Install ruby\n'
...
if [ "${INSTALL}" != true ]; then
    echo_danger "error: Cannot install \"ruby${VERSION}\", invalid version\n"
...
if [ ! -x "$(command -v rvm)" ]; then
    echo_danger "error: \"$(basename "${0}")\" requires rvm, try: 'sudo apt-get install -y rvm'\n"
    exit 1
fi
