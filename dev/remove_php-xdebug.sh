...
function is_valid_version() {
    ...
    if [ "${#VALID_VERSIONS[@]}" -eq 0 ]; then
        echo_danger 'error: "VALID_VERSIONS" array cannot be empty\n' 2
        return 1
    fi
...
function uninstall_xdebug() {
    if [ -z "$1" ]; then
        ...
        echo_success 'usage:' 2 7; echo_primary "${FUNCNAME[0]} [version]\n"
        return 1
    fi

    echo_info "sudo apt-get purge --assume-yes \"php$1-xdebug\"\n"
...
function remove_xdebug_config() {
    if [ ! -f "$1" ]; then
        ...
        echo_success 'usage:' 2 7; echo_primary "${FUNCNAME[0]} [file_path]\n"
...
if [ "$(is_valid_version "${VERSION}")" != true ]; then
    echo_danger "error: \"$(basename "${0}")\" invalid version \"${VERSION}\"" 2
    exit 1
fi

alert_primary "Remove PHP-Xdebug v${VERSION}"
