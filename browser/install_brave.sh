...
echo_warning 'Check dependencies\n'

if ! command -v curl &> /dev/null; then
  echo_danger "curl not found, install it with: sudo apt install curl"
  exit 1
fi

#--------------------------------------------------

echo_warning 'Install GPG keyring\n'

echo_info "sudo curl -fsSLo /usr/share/keyrings/${KEYRING_FILE} ${KEYRING_URL}/${KEYRING_FILE}\n"
sudo curl -fsSLo "/usr/share/keyrings/${KEYRING_FILE}" "${KEYRING_URL}/${KEYRING_FILE}"

#--------------------------------------------------

echo_warning 'Install APT sources\n'

echo_info "sudo curl -fsSLo /etc/apt/sources.list.d/${SOURCES_FILE} ${KEYRING_URL}/${SOURCES_URL_FILE}\n"
...
echo_info "sudo apt install --assume-yes ${APP_NAME}\n"
