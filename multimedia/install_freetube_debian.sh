...
VERSION=0.23.14
REMOTE_FILE=${APP_NAME}_${VERSION}_beta_amd64.deb
...
if [ -x "$(command -v curl)" ]; then
    ...
    _echo_info "curl -sSLf \"${URL}\" -o \"${DOWNLOAD_PATH}\"\n"
    curl -sSLf "${URL}" -o "${DOWNLOAD_PATH}"

elif [ -x "$(command -v wget)" ]; then
    # download latest version from github with wget
    _echo_info "wget -q \"${URL}\" -O \"${DOWNLOAD_PATH}\"\n"
