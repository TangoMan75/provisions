...
while getopts :h OPTION; do
    case "${OPTION}" in
        h) echo_label 12 'description'; echo_primary "$(basename "${0}"): Disable Wayland to allow screen sharing"
...
echo_error "Wayland is already disabled : Your session type is \"${XDG_SESSION_TYPE}\""
