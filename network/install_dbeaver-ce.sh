...
APP_NAME=dbeaver-ce
BINARY=${APP_NAME}_latest_amd64.deb
...
if [ -x "$(command -v curl)" ]; then
    _echo_info "curl -Lf \"https://dbeaver.io/files/${BINARY}\" -o \"${TEMP_DIR}/${BINARY}\"\n"
    curl -Lf "https://dbeaver.io/files/${BINARY}" -o "${TEMP_DIR}/${BINARY}"
elif [ -x "$(command -v wget)" ]; then
    _echo_info "wget -q \"https://dbeaver.io/files/${BINARY}\" -O \"${TEMP_DIR}/${BINARY}\"\n"
    wget -q "https://dbeaver.io/files/${BINARY}" -O "${TEMP_DIR}/${BINARY}"
else
