...
## jira-cli
##
## Feature-rich interactive Jira command line. jira-cli is an interactive
## command line tool for Atlassian Jira that helps you avoid the Jira UI:
## search, create, clone, link, transition issues and epics, manage sprints
## and worklogs, all from your terminal. The binary is installed as `jira`.
##
## ```
## # example usage
## jira init
...
## @link     https://github.com/ankitpokhrel/jira-cli/releases
...
if [ ! -s "${TEMP_DIR}/${REMOTE_FILE}" ] || head -n 1 "${TEMP_DIR}/${REMOTE_FILE}" | grep -q "<!DOCTYPE html>"; then
    ...
    _echo_info "Supported architectures: x86_64, arm64\n"
...
# The GitHub release archive wraps the binary in a directory named after the
# archive (without extension), e.g. jira_1.7.0_linux_x86_64/bin/jira
EXTRACTED_DIR="${TEMP_DIR}/${APP_NAME}_${VERSION}_${OS}_${ARCH}"
if [ ! -d "${EXTRACTED_DIR}" ]; then
    _echo_danger "error: Extracted directory \"${EXTRACTED_DIR}\" not found.\n"
    exit 1
fi
...
_echo_info "mv \"${EXTRACTED_DIR}/bin/${BINARY}\" \"${DESTINATION}\"\n"
mv "${EXTRACTED_DIR}/bin/${BINARY}" "${DESTINATION}"
...
_echo_warning "Run '${BINARY} init' to generate your configuration file.\n"
