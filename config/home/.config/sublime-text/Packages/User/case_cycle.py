"""
Cycle through: lower case, Capitalize, Title Case, sanitized, UPPER CASE, train-case, train-case (strip accents), snake_case, UPPER_SNAKE_CASE, UpperCamelCase, and lowerCamelCase
"""
...
class CaseCycleCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        sels = self.view.sel();
        ...
        for sel in sels:
            if not sel.empty():
                ...
                if len(string) == 1:
                    if self.isLower(string):
                        string = string.upper()

                    elif self.isUpper(string):
                        string = string.lower()

                    elif self.hasAccents(string):
                ...
                elif self.isTrain(string):
                    if self.hasAccents(string):
                        string = self.removeAccents(string)
                    else:
                        string = self.snakeCase(string, False)

                elif self.isLowerSnake(string):
                    string = self.snakeCase(string, True)

                elif self.isUpperSnake(string):
                    string = self.camelCase(string, True)

                elif self.isCapitalized(string):
                    string = string.title();

                elif self.isLower(string):
                    string = string.capitalize();

                elif self.isTitle(string):
                    if self.isDirty(string):
                        string = self.sanitize(string)
                    else:
                        string = string.upper();

                elif self.isUpper(string):
                    string = self.trainCase(string)
