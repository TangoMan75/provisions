---
name: create_remove_script
description: Scaffold a new remove_*.sh for TangoMan Provisions by reading its matching install_*.sh and adding cleanup steps for config/cache/desktop state.
---
...
The command accepts a **single argument**: a free-form prompt passed via `the user's request` (whitespace-preserved). The prompt may take any of these forms:
...
Read `the user's request`. Strip leading/trailing whitespace. If empty, ask the user to provide a name, a `category/app_name`, or a URL.
