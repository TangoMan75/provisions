...
if [ ! -x "$(command -v jq)" ]; then
    echo_danger "error: \"$(basename "${0}")\" requires jq, try: 'sudo apt-get install -y jq'\n"
    exit 1
fi

if [ ! -x "$(command -v gsettings)" ]; then
    echo_danger "error: \"$(basename "${0}")\" requires gsettings, try: 'sudo apt-get install -y gsettings'\n"
    exit 1
fi
...
function gsettingsAppend() {
    ...
    echo_info "gsettings set org.gnome.settings-daemon.plugins.media-keys custom-keybindings \"${new_bindings}\"\n"
    gsettings set org.gnome.settings-daemon.plugins.media-keys custom-keybindings "${new_bindings}"

    echo_info "gsettings set \"org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/$1/\" name \"$2\"\n"
    ...
    echo_info "gsettings set \"org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/$1/\" command \"$3\"\n"
    ...
    echo_info "gsettings set \"org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/$1/\" binding \"$4\"\n"
