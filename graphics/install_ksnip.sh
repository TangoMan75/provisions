...
if [ -x "$(command -v curl)" ]; then
    ...
    echo_info "curl -L \"${URL}\" -o \"${REMOTE_FILE}\"\n"
    curl -L "${URL}" -o "${REMOTE_FILE}"

elif [ -x "$(command -v wget)" ]; then
    # download latest version from github with wget
    echo_info "wget -nv \"${URL}\" -O \"${REMOTE_FILE}\"\n"
    wget -nv "${URL}" -O "${REMOTE_FILE}"
fi

# --------------------------------------------------

echo_info "sudo dpkg --install \"${REMOTE_FILE}\"\n"
sudo dpkg --install "${REMOTE_FILE}"

echo_info "rm -f \"${REMOTE_FILE}\"\n"
rm -f "${REMOTE_FILE}"

# --------------------------------------------------

# create shortcut on desktop
echo_info "cp -p /usr/share/applications/${PROJECT}.desktop \"$(xdg-user-dir DESKTOP)\"\n"
cp -p /usr/share/applications/${PROJECT}.desktop "$(xdg-user-dir DESKTOP)"
