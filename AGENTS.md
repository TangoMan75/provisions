...
- **Error Handling:** Do NOT use `set -e` at the beginning of scripts - let functions handle their own errors.
...
- **Required commands array**: Use an array to check multiple required commands:
  ```bash
  ...
  ```
- **Download tool detection**: Check for download tools using `[ -x "$(command -v curl)" ]` pattern:
  ```bash
  if [ -x "$(command -v curl)" ]; then
      curl -Lf "${URL}" -o "${TEMP_DIR}/${FILE}"
  elif [ -x "$(command -v wget)" ]; then
      wget -q "${URL}" -O "${TEMP_DIR}/${FILE}"
  fi
  ```
- **Download validation**: Validate downloaded files to ensure they're not empty or error pages:
  ```bash
...
- **Section separators**: Use `#--------------------------------------------------` to visually separate logical sections in scripts.
- **Comments**: Add descriptive comments throughout the script to explain what's happening.

### Logging Conventions
- **Use color helpers** from `colors.sh`:
  - `_alert_primary` for main action titles.
  - `_echo_info` for commands being executed.
  - `_echo_warning` for informational messages and process steps.
  - `_echo_danger` for errors.
- **Show commands** before running them; include `\n` at end for readability.

### Naming and Variables
- **Project metadata**:
  - `PROJECT`, `AUTHOR`, `REPOSITORY=${AUTHOR}/${PROJECT}` (lowercase, no quotes for simple concatenation).
...
**IMPORTANT! Always use the appropriate template based on the installation method required for the application.**
